...
export default {
  ...
  methods: {
    ...
    async getKanbanItemCheck() {
      try {
        this.isLoading = true
        await this.$store.dispatch(GET_ITEMCHECKS, {
    ...
    async preEditKanban() {
      try {
        ...
        await this.$store.dispatch(GET_ZONES, {
    ...
    async ActionEditKanban() {
      try {
        ...
        const data = {
          ...
          group_id: this.getKanbanDetail.group_id,
    ...
    async ActionUpdateItemCheck(itemcheck) {
      try {
        ...
        const data = {
          ...
          dest: 'ITEM_CHECKS_KANBAN',
          previous_img_paths: itemcheck.ilustration_imgs
            ? JSON.stringify(itemcheck.ilustration_imgs)
            : null, // for handled image
          ilustration_imgs: [],
        }
        console.log(data)
