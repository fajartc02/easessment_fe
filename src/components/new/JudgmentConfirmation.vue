<template>
  <CModal
    backdrop="static"
    alignment="center"
    :visible="visibleModalConfJudg"
    @close="cancelJudgmentConf()"
    size="sm"
  >
    <CModalHeader>
    ...
    <CModalBody>
      Apakah kamu yakin ingin melakukan kanban no:
      <b>{{ selectedJudgContent.kanban_no }}</b> judgment:
      <b>{{ selectedJudgContent.judgment }}</b
      >?
    </CModalBody>
    <CModalFooter>
      <CButton
  ...
  <CModal
    ...
    backdrop="static"
    alignment="center"
    :visible="visibleModalFindingJudg"
    @close="cancelFindingSubmission()"
    size="xl"
  >
    <CModalHeader>
    ...
    <CModalBody class="w-100">
      <div class="mt-3">
        <table class="table table-bordered">
          ...
          <tbody>
            <tr
              v-for="(item, index) in itemCheckKanbans"
              :key="item"
              class="align-middle"
            >
                ...
                <button
                  ...
                  @click="
                    saveScheduleCheck(
                      ...
                      item.judgment_id !==
                        'c4f5ff30-1b95-4ad8-8af8-e3e9d90bd942'
                        ? item.standart_time
                        : null,
                    )
                  "
      ...
      <CButton
        ...
        @click="
          () => {
            ...
            $emit('emit-refetch-schedule', true)
          }
        "
      >
        Close
  ...
  >
    ...
    <CForm @submit="submit" novalidate :validated="isFromFindingValidate">
        ...
        <div class="row">
          <div class="col-md-6">
            <div class="mb-2">
              ...
              <input
                type="text"
                :value="selectedJudgContent?.line_nm"
                class="form-control"
                disabled
              />
            </div>
            <div class="mb-2">
              ...
              <select
                class="form-select"
                v-model="selectedJudgContent.zone_id"
                disabled
              >
                <option
          ...
          <div class="col-md-6">
            <div class="mb-2">
              ...
              <select
                class="form-select"
                v-model="selectedJudgContent.freq_id"
                disabled
              >
                <option
            ...
            <div class="mb-2">
              ...
              <input
                type="text"
                :value="selectedJudgContent?.kanban_no"
                class="form-control"
                disabled
              />
            </div>
      ...
      <CModalFooter>
        ...
        <CButton color="primary" type="submit"> Add Finding Data </CButton>
      </CModalFooter>
    </CForm>
  </CModal>
</template>
  ...
  methods: {
    ...
    async judgmentOK() {
      try {
        ...
        this.headerForm = {
          actual_date: moment(this.selectedJudgContent.date)
    ...
    async saveScheduleCheck(
      ...
      const itemCheckKanban = this.itemCheckKanbans.find(
        (item) => item.item_check_kanban_id == itemCheckKanbanID,
      )
      const data = {
    ...
    async getScheduleCheck() {
      ApiService.setHeader()
      const detailItemCheck = await ApiService.query(
    ...
    async headerFormSubmit() {
      try {
        ...
        await ApiService.put(
          `operational/4s/sub-schedule/edit/${this.selectedJudgContent?.sub_schedule_id}`,
          this.headerForm,
        )
        return true
      } catch (error) {
    ...
    async itemCheckKanbanSubmit() {
      try {
        for (let i = 0; i < this.itemCheckKanbans.length; i++) {
          ...
          let objItemCheck = {
            judgment_id: this.judgments.find(
              (judg) => judg.text == this.selectedJudgContent?.judgment,
            )?.id,
            main_schedule_id: this.selectedJudgContent?.main_schedule_id,
            item_check_kanban_id: itemCheckKanban.item_check_kanban_id,
